...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        scheduler.add_job(
            run_investment_daily,
            CronTrigger.from_crontab(settings.INVESTMENT_DAILY_CRON, timezone=settings.TIMEZONE),
            id="investment_daily_report",
            name="开盘前投资日报"
        )
        if not settings.INVESTMENT_DAILY_ENABLED:
            scheduler.pause_job("investment_daily_report")
            logger.info(f"⏸️ 投资日报任务已添加但暂停: {settings.INVESTMENT_DAILY_CRON}")
        else:
